...
sshpass -p "1804" scp -r -o StrictHostKeyChecking=no "$directory_path" "$username"@"$host":~/pwn
echo "Success: Files copied to remote server"

sshpass -p "1804" ssh -o StrictHostKeyChecking=no "$username"@"$host" "docker cp ~/pwn/$directory_name pwn$Ubuntu_version:/root/pwn"
echo "Success: Files copied to Docker container"

sshpass -p "1804" ssh -o StrictHostKeyChecking=no "$username"@"$host" "docker start pwn$Ubuntu_version"
